package("pugixml")
    ...
    add_configs("exceptions", {description = "Enable or disable C++ exceptions.", default = true, values = {true, false}})
    ...
    on_install("windows", "linux", "macosx", function (package)
        ...
        local config_path = path.join(package:cachedir(), "source/pugixml-$(version)/src/pugiconfig.hpp")
        if not package:config("exceptions") then
            io.gsub(config_path, "// #define PUGIXML_NO_EXCEPTIONS", "#define PUGIXML_NO_EXCEPTIONS")
        end
    ...
    on_test(function (package)
        assert(package:check_cxxsnippets({test = [[
            ...
            ]]}, {configs = {languages = "c++17"}, includes = "pugixml.hpp"}))
    end)